...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lora", help="a peft adapter to merge before encoding")
...
def main(argv: list[str] | None = None) -> int:
    ...
    model = create(args.model, checkpoint=args.checkpoint, device=args.device,
                   lora=args.lora)
    identity = encoder_identity(args.model, args.lora)
    print(f"encoder identity: {identity}")
    ...
    for image_id, image_path in images:
        ...
        cached["encoder"] = np.array(identity)
